...
local function doAutoSell(setStatus)
	...
	setStatus("💰 Jual "..msTotal.." MS...",Color3.fromRGB(50,210,110))
	task.wait(0.2)
	...
	while countAllMS()>0 do
		...
		-- Method 1: fire proximityprompt langsung (tanpa E)
		firePromptNearby(CFG.SELL_RADIUS)
		task.wait(0.1)

		-- Method 2: RPC remote dengan nama item
		if rpcRE then
			pcall(function() rpcRE:FireServer("Sell", msName) end)
			pcall(function() rpcRE:FireServer(msName) end)
		end

		-- Method 3: Shoot remote (game pakai ini untuk interaksi)
		if shootRE then
			pcall(function() shootRE:FireServer(msName) end)
		end
		...
		if terjual then
			...
			failStreak+=1
			setStatus("⚠️ Gagal jual ("..failStreak.."/"..maxFail..") — Dekati NPC!",Color3.fromRGB(255,155,35))
			task.wait(1)
			if failStreak>=maxFail then
				setStatus("❌ Gagal. Dekati NPC Jual!",Color3.fromRGB(210,40,40))
				break
			end
		end
	end
	...
	unequipAll()
	if sold>0 then
		setStatus("✅ Selesai! Terjual "..sold.." MS",Color3.fromRGB(50,210,110))
	else
		setStatus("⚠️ Tidak terjual. Pastikan dekat NPC!",Color3.fromRGB(255,155,35))
	end
	task.wait(1)
end
...
local panelGrad=F(panel,C.blue,1) panelGrad.Size=UDim2.new(1,0,0,2)
...
local fpsBox=F(titleBar,C.card,4) fpsBox.Size=UDim2.new(0,94,0,24) fpsBox.Position=UDim2.new(1,-128,0.5,-12) corner(fpsBox,6)
...
local tabBar=F(farmPage,C.tabBg,3) tabBar.Size=UDim2.new(1,0,0,32)
...
-- Info: no E needed
local infoCard=F(pg1,Color3.fromRGB(13,25,13),3) infoCard.Size=UDim2.new(1,-32,0,22) infoCard.Position=UDim2.new(0,16,0,44) corner(infoCard,6) stroke(infoCard,C.green,1)
local infoL=T(infoCard,"⚡ fireproximityprompt — tidak perlu tekan E",C.green,Enum.Font.Gotham,Enum.TextXAlignment.Center,4,9)
infoL.Size=UDim2.new(1,-8,1,0) infoL.Position=UDim2.new(0,4,0,0)

line(pg1,74) secHdr(pg1,82,"BAHAN TERSEDIA")
local vW =statRow(pg1,104,"💧","Water",       Color3.fromRGB(100,200,255))
local vSu=statRow(pg1,148,"🧂","Sugar Bag",   Color3.fromRGB(255,220,100))
local vGe=statRow(pg1,192,"🟡","Gelatin",     Color3.fromRGB(255,190,60))
line(pg1,236) secHdr(pg1,244,"HASIL MASAK")
local msCard=F(pg1,C.bg,3) msCard.Size=UDim2.new(1,-32,0,52) msCard.Position=UDim2.new(0,16,0,264) corner(msCard,10) glow(msCard,C.blue,1.5)
local msBig=T(msCard,"0",C.blue,Enum.Font.GothamBold,Enum.TextXAlignment.Center,4,30)
msBig.Size=UDim2.new(0.45,0,1,0)
...
line(pg1,326)
local startW,startB=actionBtn(pg1,336,"▶  Start Auto Masak",C.blueD,C.txt)
local stopW,stopB  =actionBtn(pg1,336,"■  Stop Auto Masak", C.red,  C.txt)
stopW.Visible=false
...
local jIT=T(jualInfo,"Dekati NPC Jual — jual otomatis via fireproximityprompt.",C.txtM,Enum.Font.Gotham,Enum.TextXAlignment.Left,4,11)
...
local bIT=T(beliInfo,"Langsung beli via StorePurchase FireServer — tanpa buka dialog!",C.txtM,Enum.Font.Gotham,Enum.TextXAlignment.Left,4,11)
...
local function createFOVCircle()
	...
	inner.Size=UDim2.new(1,0,1,0) inner.BackgroundTransparency=1 inner.BorderSizePixel=0
	...
	fovCircle=circ fovStroke=s circ.Parent=sg
	return circ,s
end
...
local function getAimbotTarget()
	...
	for _,plr in ipairs(Players:GetPlayers()) do
		...
		if (hrpT.Position-hrp.Position).Magnitude>maxDistance then continue end
		local sp,onScreen=cam:WorldToViewportPoint(hrpT.Position)
		if not onScreen then continue end
		local fd=(Vector2.new(sp.X,sp.Y)-mousePos).Magnitude
		if fd<fovRadius and fd<closestDist then closestDist=fd closest=plr end
	end
	return closest
end
...
RunService.RenderStepped:Connect(function()
	...
	if rmb and aimbotTarget then
		...
		local cam=workspace.CurrentCamera
		local _,onScreen=cam:WorldToViewportPoint(bone.Position)
		if onScreen then
			cam.CFrame=cam.CFrame:Lerp(CFrame.lookAt(cam.CFrame.Position,bone.Position),math.clamp(smoothness,0.01,1))
		end
	end
end)

secHdr(combatPage,10,"AIMBOT")
local aimbotTogBtn,getAimOn=toggleRow(combatPage,30,"Enable Aimbot",false)
aimbotTogBtn.MouseButton1Click:Connect(function() task.wait(0.05) aimbotEnabled=getAimOn() end)
...
local hints={"  Tahan RMB untuk aktifkan lock","  FOV circle ngikut posisi kursor","  Merah = locked  |  Putih = mencari"}
...
line(combatPage,154) secHdr(combatPage,162,"TARGET BONE")
...
line(combatPage,228) secHdr(combatPage,236,"SETTINGS")
...
local warnCard=F(teleportPage,Color3.fromRGB(18,30,18),3) warnCard.Size=UDim2.new(1,-32,0,30) warnCard.Position=UDim2.new(0,16,0,30) corner(warnCard,8) stroke(warnCard,C.green,1)
local warnL=T(warnCard,"⚡ Step Teleport aktif — bypass anti-TP.",C.green,Enum.Font.Gotham,Enum.TextXAlignment.Left,4,10)
warnL.Size=UDim2.new(1,-10,1,0) warnL.Position=UDim2.new(0,8,0,0)

line(teleportPage,68) secHdr(teleportPage,76,"BLINK TP")
local blinkTogBtn,getBlinkOn=toggleRow(teleportPage,96,"Aktifkan Blink TP [T]",false)
local blinkEnabled=false
blinkTogBtn.MouseButton1Click:Connect(function() task.wait(0.05) blinkEnabled=getBlinkOn() end)
local blinkInfo=F(teleportPage,C.bg,3) blinkInfo.Size=UDim2.new(1,-32,0,22) blinkInfo.Position=UDim2.new(0,16,0,140) corner(blinkInfo,6)
T(blinkInfo,"Tekan T = blink maju 6 studs",C.txtD,Enum.Font.Gotham,Enum.TextXAlignment.Left,4,10).Size=UDim2.new(1,-8,1,0)

UIS.InputBegan:Connect(function(i,gp)
	if gp then return end
	if i.KeyCode==Enum.KeyCode.T and blinkEnabled and hrp then
		local cf=hrp.CFrame hrp.CFrame=cf+cf.LookVector*6
	end
end)

line(teleportPage,168) secHdr(teleportPage,176,"LOKASI")
...
-- Loading screen
local loadScreen=F(sg,Color3.fromRGB(0,0,0),999) loadScreen.Size=UDim2.new(1,0,1,0) loadScreen.BackgroundTransparency=1 loadScreen.Visible=false
local loadBg=F(loadScreen,Color3.fromRGB(8,8,12),999) loadBg.Size=UDim2.new(1,0,1,0) loadBg.BackgroundTransparency=0.2
local loadCard=F(loadScreen,Color3.fromRGB(16,16,22),1000) loadCard.Size=UDim2.new(0,280,0,120) loadCard.Position=UDim2.new(0.5,-140,0.5,-60) corner(loadCard,14) stroke(loadCard,C.blue,1.5)
local loadAcc=F(loadCard,C.blue,1001) loadAcc.Size=UDim2.new(1,0,0,2)
local lag2=Instance.new("UIGradient",loadAcc) lag2.Color=ColorSequence.new{ColorSequenceKeypoint.new(0,C.blue),ColorSequenceKeypoint.new(0.5,C.purple),ColorSequenceKeypoint.new(1,C.cyan)}
local loadIcon=T(loadCard,"🗺",C.txt,Enum.Font.Gotham,Enum.TextXAlignment.Center,1001,24) loadIcon.Size=UDim2.new(1,0,0,36) loadIcon.Position=UDim2.new(0,0,0,10)
local loadTitle=T(loadCard,"Teleporting...",C.txt,Enum.Font.GothamBold,Enum.TextXAlignment.Center,1001,13) loadTitle.Size=UDim2.new(1,0,0,18) loadTitle.Position=UDim2.new(0,0,0,48)
local loadLocName=T(loadCard,"",C.blue,Enum.Font.Gotham,Enum.TextXAlignment.Center,1001,10) loadLocName.Size=UDim2.new(1,-20,0,14) loadLocName.Position=UDim2.new(0,10,0,66)
local loadBarBg=F(loadCard,C.line,1001) loadBarBg.Size=UDim2.new(1,-32,0,4) loadBarBg.Position=UDim2.new(0,16,0,88) corner(loadBarBg,2)
local loadBar=F(loadBarBg,C.blue,1002) loadBar.Size=UDim2.new(0,0,1,0) corner(loadBar,2)
local loadBarGrad=Instance.new("UIGradient",loadBar) loadBarGrad.Color=ColorSequence.new{ColorSequenceKeypoint.new(0,C.blue),ColorSequenceKeypoint.new(1,C.cyan)}
local loadStep=T(loadCard,"",C.txtD,Enum.Font.Gotham,Enum.TextXAlignment.Center,1001,9) loadStep.Size=UDim2.new(1,0,0,10) loadStep.Position=UDim2.new(0,0,0,100)

local function showLoading(name) loadLocName.Text=name loadTitle.Text="Teleporting..." loadBar.Size=UDim2.new(0,0,1,0) loadStep.Text="Memulai..." loadScreen.Visible=true loadBg.BackgroundTransparency=1 TweenService:Create(loadBg,TweenInfo.new(0.2),{BackgroundTransparency=0.2}):Play() end
local function updateLoading(p,t) TweenService:Create(loadBar,TweenInfo.new(0.15),{Size=UDim2.new(p,0,1,0)}):Play() loadStep.Text=t or "" end
local function hideLoading() loadTitle.Text="✅ Sampai!" updateLoading(1,"Selesai") task.wait(0.5) TweenService:Create(loadBg,TweenInfo.new(0.2),{BackgroundTransparency=1}):Play() task.wait(0.2) loadScreen.Visible=false end

local isTeleporting=false
local function stepTeleport(targetPos,locName)
	if isTeleporting then return end

	-- Selalu pakai character terbaru
	local char = player.Character
	...
	task.spawn(function()
		...
		updateLoading(0.05, "Memulai...")

		-- Teleport dalam waypoints kecil (15 studs per step, 0.08s delay)
		-- Cukup lambat untuk tidak trigger anti-TP, cukup cepat untuk praktis
		local STEP  = 15
		local DELAY = 0.08
		local steps = math.max(1, math.ceil(dist / STEP))

		for i = 1, steps do
			-- Refresh char setiap step (antisipasi respawn)
			local c2 = player.Character
			local r2 = c2 and c2:FindFirstChild("HumanoidRootPart")
			if not c2 or not r2 then break end

			local alpha  = i / steps
			local newPos = origin:Lerp(target, alpha)

			r2.CFrame = CFrame.new(newPos)
			pcall(function() r2.AssemblyLinearVelocity = Vector3.zero end)

			updateLoading(0.05 + alpha * 0.88, "Bergerak... "..math.floor(alpha*100).."%")
			task.wait(DELAY)
		end
		...
		local r3 = c3 and c3:FindFirstChild("HumanoidRootPart")
		if r3 then
			r3.CFrame = CFrame.new(target)
			pcall(function() r3.AssemblyLinearVelocity = Vector3.zero end)
		end

		updateLoading(0.95, "Hampir sampai...")
		task.wait(0.2)

		-- Restore collision
		for p,col in pairs(saved) do
			pcall(function() p.CanCollide = col end)
		end
...
scroll2.Size=UDim2.new(1,0,1,-190) scroll2.Position=UDim2.new(0,0,0,190)
...
for i,loc in ipairs(LOCATIONS) do
	...
	local nm=T(row,loc.name,C.txt,Enum.Font.Gotham,Enum.TextXAlignment.Left,4,12) nm.Size=UDim2.new(0.65,0,1,0) nm.Position=UDim2.new(0,12,0,0)
